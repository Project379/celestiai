...
/**
 * MOBILE-ALPHA-REDESIGN v3 — Днес. v1/v2 chased distinctiveness; the
 * founder's actual target is familiarity (Co-Star: "feels used before").
 * See .planning/research/MOBILE_ALPHA_REDESIGN.md §0 for the full brief.
 *
 ...
 *
 * Reachable at /preview/today (dev-only entry point in (tabs)/you.tsx).
...
// (apps/web/lib/horoscope/prompts.ts: "write 4 to 6 paragraphs... each
// paragraph should develop one clear theme... end with one practical
// suggestion") — three beats, not two: an atmospheric opener, one or more
// developing paragraphs, then a payoff. The first pass here only gave
// type a way to express the payoff (weight-up on the last paragraph);
// the opener still rendered identically to the developing paragraphs in
// between, so the structure only half-showed. Now all three beats read
// distinctly, each via ONE lever, not stacked ornament:
...
// the AI prompt spec (apps/web/lib/horoscope/prompts.ts) asks for "2 to 4
// of the most important active or building influences," one per
