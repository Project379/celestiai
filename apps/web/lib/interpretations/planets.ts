...
export function getRisingInterpretation(sign: string, degree?: number, isApproximate?: boolean): InterpretationData {
    ...
    return {
        title: isApproximate ? 'Приблизителен възходящ знак' : 'Възходящ знак',
        position: formatPosition(sign, degree),
        brief: SIGN_BRIEF[signKey] || '',
        overview: `Възходящият знак описва първото впечатление, езика на тялото, инстинктивния подход и атмосферата, която носите в нови ситуации. ${SIGN_INFLECTION[signKey]} Често това е първият слой, който другите забелязват, преди да усетят по-дълбоката история на Слънцето и Луната.${isApproximate ? ' Понеже часът на раждане не е точен, приемайте тази насока като ориентир, а не като абсолютна сигурност.' : ''}`,
        strengths: [
        ...
        growth: 'Използвайте възходящия знак като вход към света, но не като цялата си самоличност. Той е начинът, по който започвате, ориентирате се и се появявате, но става най-силен, когато е свързан с вътрешната ви истина.',
        aspectInsights: [],
    }
}
