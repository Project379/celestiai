...
## 4. Derived astrological data, and the transfer to the LLM

### 4a. What we compute and store
...
### 4b. What we send to the LLM provider — **exactly**

Readings are generated by an LLM accessed through **OpenRouter**
(openrouter.ai), which routes to an underlying model provider. **VERIFIED
by reading the request-construction code** (`lib/oracle/chart-to-prompt.ts`,
`lib/oracle/prompts.ts`, `app/api/oracle/generate/route.ts`,
`app/api/horoscope/generate/route.ts`):
...
**NOT sent to OpenRouter:** the user's name, email, Clerk id, the chart's
`name` label, the raw birth **date**, the raw birth **time**, the raw
birth **place** (city / coordinates), any device or session identifier.
The request carries **no `user` parameter and no identifying headers**
(VERIFIED — `lib/ai/client.ts` configures only `baseURL` + API key).
...
### 4c. Open items for counsel on the LLM transfer

- **OpenRouter is US-based**, and the underlying model provider may be
  elsewhere again. This is a **third-country transfer** needing a named
  sub-processor entry and a safeguard (SCCs / DPF). Counsel to advise on
  disclosing "OpenRouter + the set of downstream model providers it may
  route to" vs. pinning a single provider.
- **Data-retention / training at OpenRouter and downstream.** OpenRouter
  offers per-request and per-account data-policy controls (including a
  zero-data-retention / no-logging mode and provider allow-lists).
  [FOUNDER DECISION / ACTION] we should configure OpenRouter for
  **no prompt logging / ZDR** and a **no-training** provider set, then
  the policy can state that. Counsel to advise what the policy must say
  if we cannot fully guarantee downstream non-retention.
- The **current model is a placeholder** (Llama 3.3 70B) and **will
  change before or shortly after launch.** The policy should describe the
  processing by role ("an LLM provider accessed via OpenRouter"), not by
  model name, so a model swap isn't a policy change.
...
[FOUNDER DECISION] the minimum age (Bulgaria's digital-consent age under
Art. 8 is **14**). State the minimum age and that we do not knowingly
process data of anyone below it.
...
| **OpenRouter** (+ downstream model providers) | LLM inference for readings | US (+ varies) | derived natal-chart text + topic (no direct identifiers) — §4b |
...
1. **Legal basis for birth data:** contract (Art. 6(1)(b)) vs. consent.
2. **OpenRouter transfer:** how to name the sub-processor(s); what to say
   about downstream retention/training if we cannot fully guarantee ZDR.
3. **`audit_logs` residual retention** after deletion — lawful under
   Art. 6(1)(f) with a stated period, or must it be deleted too?
4. **Payment-record retention** — does BG accounting law oblige *us* to
   keep anything after deletion (accountant question)?
5. **Кръг third-party birth data** — the basis and disclosures for a user
   entering another person's birth data.
6. **Art. 22 / profiling** — is our "not a significant decision" position
   sound; anything needed for the "health" topic?
7. **Art. 9** — anything beyond "we don't solicit it; don't enter it"
   for free-text fields and the health topic.
8. **Cookie-consent** — confirm no banner needed (strictly-necessary
   only), for Bulgaria specifically.
9. **Minimum age** (14 in Bulgaria) and how the policy should frame
   enforcement.
10. Whether an **Art. 30 record of processing** and/or a **DPIA** is
    advisable given the birth-data-at-scale + AI-transfer profile (we
    suspect a DPIA is at least worth doing).
...
[FOUNDER DECISION] are business/legal choices not yet made. The current
LLM is a pre-launch placeholder and will change — describe the processing
by role, not by model.*
