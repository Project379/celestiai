...
**Also found, not yet fixed:** `font-cinzel` on Cyrillic text in `rhythm.tsx`, `circle.tsx`, and `you.tsx` (REVISIT-42's bug — Cinzel has zero Cyrillic glyphs) during this same audit. Out of scope for the navbar-legibility pass; flagged here so it isn't lost. `NatalWheelLegend.tsx`'s instance was fixed in this pass (Карта's «i» sheet investigation).
