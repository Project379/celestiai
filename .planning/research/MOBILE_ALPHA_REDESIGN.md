...
**Correction 2026-08-03 — decided and shipped, not still pending.** `NatalWheel.tsx` today uses a single tap-surface (not per-planet `Pressable` overlays) with `HIT_RADIUS_MIN = 22` px (a 44pt-diameter minimum, matching the HIG floor this section names), `hitRadius = Math.max(HIT_RADIUS_MIN, size * 0.063)`, with an in-code comment citing this redesign doc by name and a documented nearest-planet-within-radius lookup for stellium disambiguation. This section's framing as an open scope decision is stale; check `NatalWheel.tsx:109-113` for current behavior rather than this paragraph.
...
### 0.3 Daily-reading length — ratified 2026-07-22, rewritten, held pending model decision (correction 2026-08-03)
...
**Decision (as originally ratified 2026-07-22):** cut the prompt FORMAT spec (`apps/web/lib/horoscope/prompts.ts:18-23`) from "4 to 6 paragraphs, 2 to 4 influences" to "exactly 2 short paragraphs, 400 to 550 characters, do not exceed 550, 1 influence (2 only if truly load-bearing)." VOICE AND TONE (lines 4-8) untouched — this is a length/coverage cut, not a voice change. Paired with a client-side expand safety net (`preview/today.tsx`'s `EXPAND_THRESHOLD_CHARS = 900`) for variance and for today's-already-cached long readings, so an outlier never breaks the screen.

**Correction 2026-08-03 — the FORMAT spec was rewritten again, later, past what this section describes.** `apps/web/lib/horoscope/prompts.ts` today specifies **3 short independent lines** (headline theme / where it shows up / one practical suggestion), not the 2-paragraph structure above — landed alongside the i18n workstream (commit `35472a4`, "3-line horoscope format"). This IS the prompt the code sends on every generation today; it is not inert or feature-flagged off. But per founder direction, don't read that as a settled, validated spec: paragraph/length/structure adherence is model-specific (point below), the production model (`meta-llama/llama-3.3-70b-instruct`) is independently known-broken for Bulgarian generation quality (`.planning/i18n/MODEL_CAPABILITY_LOG.md` — non-words, Russian drift), and re-verification against whatever model REVISIT-45 lands on is required before this format spec should be treated as final. Status: **rewritten and executing, held/unvalidated pending the model swap** — not "landed" in the sense of confirmed-correct.
...
**Tab bar** (`(authed)/(tabs)/_layout.tsx`): `useSafeAreaInsets()` replaces the hardcoded `height:72`/`paddingBottom:18` (mirrors commit `6025f58`'s top-inset treatment); `tabBarActiveTintColor` now reads `color.amber` from `tokens.ts` instead of a drifted `#fcd34d` (**correction 2026-08-03: superseded again — current code reads `color.starlight`, not `color.amber`, per the later warm/cool token work; check `_layout.tsx:120` rather than this line**); labels off Cinzel (Cyrillic text — REVISIT-42's exact bug, noted there that this surface was outside its original reach); 5 tracked-caps labels collapsed to plain sentence case; 5 custom-SVG icons added (`components/design-system/TabIcons.tsx`) — founder-reviewed at real 20/24/28px scale before wiring in, icon+label retained per iOS convention.
...
**Correction 2026-08-03:** this table is a snapshot from 2026-07-23 audit time, already superseded on at least one row — `LunarPhaseCard.tsx` is 30/43 `fontFamily:`-set sites today (confirmed by direct grep), not the 1/43 recorded above. Don't treat this table as current status for any row without re-checking the file; it's a historical measurement, not a live tracker.
...
## 24. Premium pass — items #1–3 ratified and landing, proof surface authorized (2026-07-24)

Founder interview resolved the premium-feel brief (Co-Star/Phantom/Weather/X for structure, Balatro/tarot-stock/dark-fantasy-game-UI for material and motion — full research + ranked proposal delivered as an artifact, not duplicated here). Five research passes plus a direct precision audit of `apps/mobile` found the headline issue: of the app's ~79 `Pressable` usages, exactly one component (`NavRow.tsx`) gave any visual confirmation of a tap. Items #1–3 below are ratified and shipping this round; items #4–7 (elevation-token normalization, radius-scale normalization, hairline-opacity normalization, a gesture-tied foil-sweep edge treatment) stay **unratified pending the founder's device pass on #1–3** — deliberately not bundled in, since #4–6 are cumulative-precision work with no single perceptible moment and #7 risks reading as a new decorated element (R3).

**Item #1 — universal press feedback.** `pressFeedback(pressed)` in `components/design-system/tokens.ts`: opacity 0.6 + scale 0.97 on press, two axes so the signal clears the R7 calibration bar rather than repeating a single-degree-step mistake. Applied across the app's visible Pressables; invisible full-screen hit-surfaces (`NatalWheel.tsx`'s tap-detection overlay and its disambiguation-dismiss backdrop) are deliberately excluded — there's no visible content for a press state to act on.

**Item #2 — haptics wiring.** **Policy, stated explicitly rather than left implicit**: the press-feedback primitive does **not** auto-fire haptics. Every haptic call is wired individually at a specific, meaningful interaction site (selection/impact/notification per Apple's own semantic taxonomy — see the research artifact's mapping table), never attached generically to "every Pressable." Auto-firing from the shared primitive was considered and rejected — it is exactly the overuse failure the haptics research warned against (haptic fatigue, users disabling the feature entirely once it stops signaling anything specific). Mapped interactions: chart-wheel drag (`selectionAsync`), planet select (`impactAsync(Medium)`), sheet open (`impactAsync(Soft)`), card/crystal reveal (`impactAsync(Rigid)`), reading/draw completion (`notificationAsync(Success)`), favorite toggle (`impactAsync(Light)`), and — added this round, filling a gap the founder flagged as conspicuous next to a Success mapping — **failed generation, failed save, and network errors** (`notificationAsync(Error)`). `expo-haptics` is an existing, already-precedented dependency (`TimePicker.tsx`), not a new one.

**Item #3 — card press-scale + lift.** Reanimated `withSpring` scale/translateY on press for the app's card-class components, neutral (uncolored) shadow that deepens with the lift — colored glow reserved for the one proof surface only, per the R4 resolution in the research artifact (card lift must not spend a second accent-color role on screens that already use amber elsewhere).

**HT8 amendment** (the "drop entry animations on data-display screens" discipline pattern, `HANDOFF-2026-05-09.md`) — narrowed to a REVEAL-mechanic exception, not a frequency-based one; see that document for the full wording. Currently covers only `CrystalOfTheDayCard.tsx`.

**Proof surface**: `CrystalOfTheDayCard.tsx` (`apps/mobile/components/crystals/`, shown on `you/crystals.tsx`) — a daily crystal draw, structurally a tarot pull, currently a static `View` with zero interactivity. Full treatment: Surface1 panel + hairline border, press-scale 0.97 with spring settle, gem lift-and-rotate on press, `impactAsync(Rigid)` on release, and a once-daily reveal entrance animation under the HT8 exception above — verified to fire once per actual daily draw, not on every render or navigation back to the screen (the exact cost HT8 exists to prevent).

**Order**: items #1–3 ship as their own commit(s), independent of the proof surface, which lands on top afterward. Founder device-tests both together. Halt after push for that pass — items #4–7 stay open pending it.
...
## 25. Doc reconciliation — Днес's hero is the moon alone; the phase name is a tracked eyebrow, not a second hero (Stage 2, 2026-07-27)

**This doc's own §4.1/§4.2/§14 record (the "40px hero phase name", the "hero-text:caption ≈3.6×" table row, `tokens.ts`'s 32px `hero` tier) has been superseded by the ratified Stage 2 mockup** (`.planning/design/mockups/dnes-v4.html`, `.phase-mark`) and is corrected here rather than left to silently disagree with the shipped screen. The mockup renders the lunar-phase name as a 9.5px tracked-caps eyebrow (`.28em` letter-spacing, starlight, .82 opacity) directly under the 202px moon — not a 32-40px second headline competing with the glyph for the R1 slot.

**What changes, concretely:**
- The 202px moon (52% of a 390px frame) is the ONLY hero on Днес — matching this doc's own §21/§22 precedent on Ритъм (glyph leads, text is secondary), which §4.1's original framing already argued for and the phase-name-as-hero implementation never actually matched.
- `tokens.ts`'s `hero` (32px) tier is removed — it had exactly one consumer (this phase-name slot), and that slot no longer wants a hero-scale text tier. A new `eyebrow` tier (9.5px, 2.66px tracking = `.28em` computed, not eyeballed) replaces it.
- **This is Днес's one R3-reserved tracked-caps use** — the eyebrow slot §26 of this doc's own tracked-caps inventories elsewhere (§20's rhythm.tsx audit, §23's typeface-conversion table) already treat as the standing budget per screen. Nothing else on Днес gets tracked caps; this is that screen's one.
- §4.1/§4.2's glyph:caption ≈13.6× ratio claim stays true (the moon is still the size-contrast element); the hero-text:caption ≈3.6× row is now describing a tier that no longer exists on this screen and should be read as historical, not current.

**Why this wasn't caught earlier**: the phase-name-as-32px-hero was implemented and shipped (Round A cutover, 2026-07-22) before the mockup artifact (`_source-v4.html`) existed as a committed, checkable spec — this doc recorded the implementation's own reasoning at the time, which was internally consistent but never re-verified against a design source once one existed. Stage 2's investigation (extracting and verifying the mockup files) is what surfaced the disagreement.

## 26. REVISIT 60 — glyph-sheet tarot card-back flip animation, deferred pending Phase 0 close + perf fix

**Founder decision (2026-07-27, device pass):** the glyph-tap sheet (`PlanetDetail.tsx`) is getting a new-language/cool-surface visual pass (Stage 2 item 6) once the app-wide UI-thread perf regression (10-15 FPS, all screens, JS pinned at 60 — see the perf bisection log for this pass) is fixed and the visual pass is verified not to regress the one screen currently smooth. A tarot-style card-back flip-to-reveal animation for that same sheet was proposed and explicitly NOT built this round: **"Adding an animation to an app that's already dropping frames makes both problems harder."**

**Scope, once unblocked:** card back appears, spins once to reveal the reading — a single motion, not a loop; same HT8-exception discipline as `CrystalOfTheDayCard.tsx`'s reveal animation (§24) — fires once per genuine reveal, not on every render/reopen. Needs its own investigation table (motion spec, Reanimated worklet cost on a sheet that's about to carry a heavier visual surface, whether it's additive to or replaces the existing slide-up `Modal` transition) before implementation — not scoped further here.

## 27. Doc reconciliation — nav-fade ceiling raised to .92 (systemic rule, device pass, 2026-07-27)

**The mockup's `.nav-fade` (`_source-v4.html`) specifies `rgba(8,6,15,.6)` fading to 0 over 78% of the band.** Shipped 1:1 initially. Device pass found scrolled reading text stayed legible under the tab bar — the mockup's own ceiling never lets more than 60% of the underlying color apply, so up to 40% of whatever's behind it always shows through. A static mockup screenshot never exercised this against live scrolled text, so the value was never actually tested against the case it exists to solve.

**What changed:** ceiling raised to `.92`, ramping to transparent over 85% of the band (`(tabs)/_layout.tsx`'s `NavFade`). Promoted to a systemic rule, not a one-off tweak: no text or glow is ever legible under persistent chrome. The same recipe is now also used for the fade `ScreenShell.tsx` renders above a `pinnedBottom` block (Днес's invite), since scrolled content approaching a pinned element has the identical failure mode as content approaching the tab bar. Audited every tab screen for the underlying gap while at it: Ритъм/Кръг/Ти never used `ScreenShell` at all (their own `SafeAreaView`+`ScrollView`, flat `paddingBottom: 120`, ~30px real tab-bar clearance vs. the ~52px target) — brought in line with the same formula.

**Also found, same day, and fixed the same day (correcting this entry, 2026-08-03):** `font-cinzel` on Cyrillic text in `rhythm.tsx`, `circle.tsx`, and `you.tsx` (REVISIT-42's bug — Cinzel has zero Cyrillic glyphs) during this same audit. Despite the "not yet fixed" framing originally written here, all three carry a dated "REVISIT-42 fix (2026-07-27)" comment in the shipped code and have zero remaining `font-cinzel` classes — confirmed by direct grep, not assumed. `NatalWheelLegend.tsx`'s instance was fixed in this pass too (Карта's «i» sheet investigation).

**Sequencing, explicit:** (1) perf bisection lands and UI FPS is back to normal across all screens including this one, (2) item 6's visual rebuild lands and is re-measured to confirm it didn't reintroduce the regression on the one screen that was smooth, (3) only then does this REVISIT get its own investigation + plan. Building it earlier compounds an unfixed perf problem with a new animated surface on the exact screen currently used as the "this is what smooth looks like" baseline.

## 28. Карта scroll stutter — UNFIXED, open issue (correction 2026-08-03)

**Status check, since nothing in this doc previously said so plainly:** the redraw/FPS problem (item 6 above, `perfDebug.ts`'s bisection) is resolved — confirmed on-device at 58-60 UI FPS with all four animations (star twinkle, moon glow pulse, breathing embers, wheel graticule resolve-in) running live, via moving the animated value from SVG-internal props to wrapping `Animated.View` opacity (see `AmbientBackground.tsx`, `MoonGlyph.tsx`, `motion.ts`). `perfDebug.ts` and its 9 call sites were removed 2026-08-03 once this was confirmed — the flags had done their job.

**That is a separate bug from Карта's scroll stutter, and the scroll stutter is NOT fixed.** The attempted fix — `shouldRasterizeIOS`/`renderToHardwareTextureAndroid` on the wheel, deferred until `WheelArrivalContainer`'s arrival animation settles (see that file's `onSettled` doc comment) — was tried, instrumented, and confirmed dead on-device: both props were verified applied, and the stutter persisted regardless. Root cause per `chart.tsx`'s own comment (2026-07-27, item 4): this app runs React Native's New Architecture (`app.json` — `newArchEnabled: true`), and these are legacy-architecture view-manager props with a documented history of being inert under Fabric. The dead rasterization call was removed rather than left as a no-op, and the comment says a "view-shot investigation" is the next candidate fix — **not yet built.**

**Correcting a residual staleness this same pass:** `WheelArrivalContainer.tsx`'s `onSettled` doc comment still describes "the caller uses this to defer rasterizing the wheel" in the present tense, as if that mechanism is still active. It isn't — `chart.tsx` no longer calls it for that purpose. Flagging here rather than editing that comment now, since I wasn't asked to touch that file this pass.

**No REVISIT tracks this today** — checked `REVISIT-TRIGGERS.md`, no entry for the scroll stutter or the view-shot candidate fix. Карта's scroll performance is a known, real, currently-unsolved problem, not something to assume closed because the unrelated redraw/FPS bug got fixed in the same investigation window.
