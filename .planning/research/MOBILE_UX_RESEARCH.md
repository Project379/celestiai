...
- **Not web-first:** push, home-screen widgets, and OS-level notification scheduling are native-only. Every week spent on web-only features is a week those wedges don't exist. [verified: web push works but lacks widgets and OS scheduling; Duolingo 23.5hr pattern requires background scheduling unavailable on web] **Correction 2026-08-03:** the Duolingo-style 23.5hr per-user pattern-time scheduling described here and at §10 item 6 remains [planned]/aspirational only — it has not been built on either platform. P.16 (closed 2026-08-03) shipped native push delivery matching web's existing blanket, non-personalized daily-horoscope timing (same message, same 06:00 UTC cron, all users), not per-user pattern-time. Don't read this section as describing shipped behavior.
