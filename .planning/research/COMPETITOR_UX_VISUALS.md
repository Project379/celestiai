# 🔮 Competitor UX/Visual Teardown vs. Stellaeum MVP

This document breaks down how current top-tier astrology apps handle the core features defined in the **Stellaeum v1.0 Feature Requirements**, outlining competitive visual patterns and how Stellaeum's UI/UX differentiates itself.

---

## 1. Onboarding & Birth Data Wizard 
*(Stellaeum components: `BirthDataWizard.tsx`, `DateStep.tsx`, `TimeStep.tsx`, `LocationStep.tsx`)*
...
* **The Stellaeum Difference:** Co-Star’s onboarding is famously anxiety-inducing and brutalist. Stellaeum uses a **glassmorphism, ambient space** aesthetic. The `BirthDataWizard.tsx` guides users with a supportive tone, making the complex data-entry (location autocomplete via `/api/cities/search`, time adjustments) feel premium and expansive, not like a terminal prompt.
...
* **The Stellaeum Difference:** Stellaeum avoids the "cheap" look of overly bright gamified widgets, opting for deep, grounded, realistic cosmic aesthetics (e.g., using React Native Skia on mobile and Tailwind CSS variables on web) to convey **accuracy and trust** (+ Swiss Ephemeris backing) to the Bulgarian market.

---

## 2. The Natal Chart / Wheel Display 
*(Stellaeum components: `components/chart/NatalWheel.tsx`, `NatalWheelSkia.tsx`)*
...
* **Wireframe Pattern:**
  ...
  +-----------------------+
  ```
* **The Stellaeum Difference:** Bulgarians seeking astrological insight actually care about the math (degree accuracy, transits). By utilizing a mathematically accurate D3/Skia wheel (`NatalWheel.tsx`) calculated via `sweph`, **Stellaeum visually proves its precision.** Sanctuary dumbs it down; Stellaeum makes it accessible via interactive tooltips (`PlanetDetail.tsx`) while keeping the impressive visual mathematics intact.
...
* **The Stellaeum Difference:** The Pattern strips away the "magic" entirely. Stellaeum embraces the "magic" but delivers it through high-end, premium data visualization. 

---

## 3. Daily Horoscope / Notifications
*(Stellaeum components: `DailyHoroscope.tsx`, `PushNotificationBanner.tsx`, `api/cron/daily-horoscope`)*
...
* **The Stellaeum Difference:** Stellaeum's `DailyHoroscope.tsx` functions as a **"Wise Friend"**. Instead of trying to be edgy (which would culturally alienate the Bulgarian market), Stellaeum uses the AI SDK (`streamText`) to generate supportive, actionable daily guidance grounded in actual Bulgarian cultural nuances. Visually, it uses soft markdown typography with inline planet icons.

---

## 4. AI Oracle / On-demand Readings
*(Stellaeum components: `hooks/useOracleReading.ts`, `api/oracle/generate`, `ai_readings`)*
...
* **The Stellaeum Difference:** Stellaeum respects the user's intelligence (critical for Bulgarian consumer trust, currently at 31% active distrust). `PricingContent.tsx` heavily leverages the freemium strategy: *Free → "This is deeply accurate" → "I want the locked topics" → Premium*. Stellaeum's paywall centers on clear typography, the stark €6.99/mo (EUR adoption) value proposition, and a transparent 7-day trial feature (`PAY-08` via Stripe). No fake timers. No dark patterns.
