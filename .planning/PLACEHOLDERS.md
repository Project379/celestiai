...
| THINKING-BUDGET-SPIKE | Gate 9 (2026-09-04, paid tier): 1 of 20 calls (5%) spent 867 thinking tokens against the `maxOutputTokens: 900` ceiling, leaving 18 tokens for the actual answer, and threw `AI_NoOutputGeneratedError`. Two fixes applied 2026-09-04: (1) `thinkingConfig.thinkingBudget: 300` — NOTE the Gemini API rejects setting both `thinkingLevel` and `thinkingBudget` together, so `thinkingLevel: 'low'` was removed, not kept alongside it; (2) `isTransientAIError` now recognizes `AI_NoOutputGeneratedError` by name, AND (the actual bug, found live) `generateFinalText`'s `callModel` now forces the lazy `.output` getter to throw *inside* the try/catch — it previously threw after, where neither classifier was ever consulted and the fallback never fired regardless of (2) alone. **Re-run twice to verify, both still red:** the retry fix is CONFIRMED WORKING (the fallback to 3.6-flash visibly engages in both runs' logs) — but the cap does NOT reliably hold, and the fallback model hits the identical failure. Both runs failed on the SAME chart (id 6, Burgas) on BOTH the primary and fallback model — this is not the random ~5%-of-any-chart pattern first assumed; across all 5 clean live attempts to date (2 pre-fix, 3 post-fix) Burgas has failed 3 times and succeeded 2, suggesting something about that specific chart's aspect data (not model non-determinism alone) drives unusually high thinking spend. One intermediate measurement (before the getter fix, budget-only) logged 549 thinking tokens against the configured 300 cap on this same chart — the cap is not a strict server-enforced ceiling. **New gap introduced by the getter fix:** `logAiUsage` is only reached on a successful `callModel` return, so a failing call's real token counts are no longer observable at all — Burgas's two post-fix failures have no usage telemetry, unlike the original 867-token measurement. Stays OPEN; the retry mechanism works, but neither the root cause nor the cap is actually resolved, and failure-path visibility regressed | CODE | Petko | Launch quality bar | OPEN | | apps/web/lib/ai/generate-final-text.ts |
