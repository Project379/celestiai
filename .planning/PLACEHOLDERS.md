...
| LLM-MODEL-SWAP | LLM-MODEL (decision) is RESOLVED and the implementation HAS landed — production now calls Gemini (`gemini-3.7-flash`, falling back to `gemini-3.6-flash`; see SYSTEM-MAP §4), not Llama, on this branch (`gemini/rebased-onto-injection`, not yet merged to main). UPDATED 2026-09-04: the paid-tier key cleared GEMINI-API-TIER's quota blocker — two full Gate 9 runs (20 calls total) both generated cleanly (10/10, then 9/10 with one non-quota throw; see THINKING-BUDGET-SPIKE), so quality data that was uncollectable for three straight sessions now exists. Gender agreement is clean (0/10 errors both runs — the Llama-era 8-9/10 failure looks resolved). **Stays OPEN on what's left:** (1) phrase repetition is not resolved — the sentinel-example rewrite cleared the specific opening construction it targeted, but a new mid-text 3-word pattern failed the aggregate check both runs (see GATE9-PHRASE-REPETITION; threshold ruling pending); (2) SAFETY-FILTER-UNTESTED — health/death/relationship-crisis content has never been run against Google's policy; (3) the branch itself is still unmerged. No longer stuck on missing quality data — that specific blocker is cleared | CODE | Petko | Launch | OPEN | | apps/web/lib/ai/client.ts |
...
| THINKING-BUDGET-SPIKE | Gate 9 (2026-09-04, paid tier): 1 of 20 calls (5%) spent 867 thinking tokens against the `maxOutputTokens: 900` ceiling, leaving 18 tokens for the actual answer, and threw `AI_NoOutputGeneratedError` — non-deterministic (an isolated retry of the identical prompt completed cleanly with 0 thinking tokens logged). `thinkingLevel: 'low'` (`generate-final-text.ts`) signals a preference, not a hard cap; `@ai-sdk/google`'s `thinkingConfig.thinkingBudget` (a numeric token cap) is unset. Traced: this error is caught by neither `isTransientAIError` (no `statusCode`/`isRetryable`) nor `isUpstreamAiError` (`AI_NoOutputGeneratedError` isn't in that name allowlist) — `generateFinalText`'s same-provider fallback to 3.6-flash never fires, `route.ts`'s outer 2-attempt quality-retry loop is broken by the raw throw, and it surfaces as a bare 500 (`refundClaim()` does run first, so quota isn't burned). Cost: the spike call cost €0.00424 vs a €0.00256 normal-call average — 65% more expensive when it fires (see THINKING-TOKEN-COST) | CODE | Petko | Launch quality bar | OPEN | | apps/web/lib/ai/generate-final-text.ts |
