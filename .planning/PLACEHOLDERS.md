...
| NEVER-EXPIRES-SENTINEL | `'2999-12-31T00:00:00.000Z'` duplicated between route.ts and the restore SQL in TIER-DEFINITION-2026-09-01.md §12; drift silently expires lifetime readings | CODE | CC | — | OPEN | | apps/web/app/api/oracle/generate/route.ts |
...
| NEVER-EXPIRES-SENTINEL | TIER-DEFINITION §12 (the restore SQL); `apps/web/app/api/oracle/generate/route.ts` (the constant) | **the code** (`NEVER_EXPIRES_AT` in route.ts) is SoT for the *value*; TIER-DEFINITION §12 must be updated whenever it changes. This register tracks the duplication risk. | A literal duplicated between code and a SQL snippet has no doc SoT — only a "these must match" note, which is what the marker is for. |
