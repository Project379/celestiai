...
**Guarded-navigation is canonical for every Pressable → router.push call-site.** P.7-fix found rapid double-tap firing `onPress` twice before the screen transition absorbs the second tap, pushing two screens (founder-reproduced on Ти tab section rows). Fixed via `apps/mobile/hooks/useGuardedNavigation.ts` — a `useRef` in-flight guard wrapping expo-router's `push`, clearing itself 600ms after the call so a legitimate back-and-re-tap still navigates. Applied at all 11 call-sites found by inventory (chart, index, rhythm ×2, you, wizard date/time/location, OracleEntry, StoriesContent). Going forward: new Pressable → router.push wiring must go through `useGuardedNavigation`'s `push`, not `useRouter().push` directly; a bare `router.push` inside a `Pressable onPress` is a review-time defect. `router.back()`/other router methods are unaffected and keep using `useRouter()` directly.
