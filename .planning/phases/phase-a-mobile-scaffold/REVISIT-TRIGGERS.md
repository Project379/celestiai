...
## 67. Zero brand imagery anywhere in the repo — third EAS preview build failure, splash-screen plugin removed as a stopgap, real assets still needed

**Status:** Filed 2026-08-05. Third `--profile preview` Android build got past the Sentry-upload and native-compile stages and failed at `:app:processReleaseResources` with `error: resource drawable/splashscreen_logo not found`. Traced to source, not assumed: `app.json`'s `expo-splash-screen` plugin config was `{"backgroundColor": "#08060f"}` only, no `image`. Read the actual installed `@expo/prebuild-config@54.0.8` and `54.0.9` (both patch variants present in the lockfile — same bug in both) plugin source line by line:

- `withAndroidSplashImages.js` only writes a `splashscreen_logo.png` drawable per-density `if (image)` — with no `image` configured, nothing is ever written, correctly and silently.
- `withAndroidSplashStyles.js`'s `addSplashScreenStyle`, however, **unconditionally** (for any non-legacy plugin config, which is *always* the case when the plugin is present at all — traced the `isLegacyConfig` computation through `withSplashScreen.js` → `withAndroidSplashScreen.js` and confirmed it can never evaluate true via the plugin route, only via the fully-deprecated top-level `config.splash`/`config.android.splash` keys with no plugin at all) writes `windowSplashScreenAnimatedIcon = @drawable/splashscreen_logo` into `styles.xml`, regardless of whether any image was configured.

The two together mean: as soon as `expo-splash-screen` is listed as a plugin at all, SDK 54's current prebuild-config always references a drawable it only sometimes generates — `image` is documented as optional but is not, in practice, optional for Android under this SDK line. This is an upstream Expo gap, not a project misconfiguration, and — consistent with everything else surfaced this week — it never showed up before because Expo Go supplies its own splash screen and this was the first build to run the real native asset-linking pipeline at all.

**No image exists to point at.** Searched the entire repository, not just `apps/mobile`: zero `.png`/`.jpg`/`.jpeg`/`.svg`/`.webp` files anywhere outside `node_modules` (confirmed via `find`, twice, across the whole tree). `apps/mobile/assets/` contains fonts only. `apps/web/public/` contains only `sw.js` — no favicon, no logo. There is nothing to point the splash config at, and per explicit founder instruction, no placeholder artwork was invented.

**Fix applied:** removed the `expo-splash-screen` plugin entry from `app.json`'s `plugins` array entirely (kept the bare backgroundColor config would have hit the same bug regardless — traced, not assumed, that no config shape avoids it while the plugin is present). This is a real fix, not a workaround around a config value — it stops the broken code path from running at all rather than trying to satisfy it. Confirmed safe: `apps/mobile/app/_layout.tsx` calls `SplashScreen.preventAutoHideAsync()`/`hideAsync()` from the JS API, which works via the autolinked native module independent of whether the config plugin ran — only the plugin's visual/theme setup (branded dark background, no image) is lost, not the show/hide behavior itself. `npx expo config --json` resolves cleanly with the plugin removed. **Trade-off, stated plainly: this build will show Android's default splash (unbranded, likely a plain flash) instead of the intended dark `#08060f` background, until real splash artwork exists and the plugin is reinstated with an `image` supplied.**

**Full asset-pipeline audit, run now rather than found across three more builds (per founder instruction):**
- **App icon / adaptive icon:** not configured anywhere (`app.json` has no `icon`, `android.icon`, or `android.adaptiveIcon` keys). Checked `withAndroidIcons.js` — unlike the splash plugin, this one properly early-returns (`if (!icon) return config`) when nothing is configured, so it's safe from this specific failure mode. The app will build and run with Expo's generic default icon, not a Stellaeum-branded one — a real product gap for eventual store submission, but not a build blocker today.
- **Notification icon:** not configured, and `expo-notifications` isn't even listed in `app.json`'s `plugins` array despite being a dependency — so no notification-icon-specific native resource generation runs at all, safe from this failure mode for the same reason (nothing references what isn't configured). Push notifications will show Android's default bell icon rather than a branded one.
- **Favicon (web):** confirmed absent — no `favicon.ico` under `apps/web/app/` (Next.js file convention) or `apps/web/public/`. Unrelated to this Android build, doesn't block it, but is the same underlying gap.

**The actual gap, named plainly:** this project has never had any brand imagery — icon, splash logo, notification icon, or favicon — designed or committed, on either platform, at any point. Today's fix unblocks the build; it doesn't close the gap. A real app icon (Play Store requires one at submission; Expo's generic default will not pass review) and ideally a splash logo are needed before this app can ship anywhere beyond an internal preview APK.

**Trigger:** Before any App Store or Play Store submission (hard blocker — Expo's default icon will not pass store review). Sooner if the founder wants the preview build to look like Stellaeum rather than a generic Expo app during device testing. Re-add the `expo-splash-screen` plugin with a real `image` path once artwork exists; re-add `icon`/`android.adaptiveIcon` config at the same time since they're the same underlying asset-creation task.

**What's needed, concretely:** at minimum a square logo mark (recommend 1024×1024 transparent PNG) usable as both the splash centerpiece and the adaptive-icon foreground layer; the existing `#08060f` dark background already works as the icon/splash background color without needing a separate background image. A monochrome variant is optional (only needed for Android 13+ themed icons). Not something to generate here — flagging for founder/design decision per [[feedback_no_ui_work]].
