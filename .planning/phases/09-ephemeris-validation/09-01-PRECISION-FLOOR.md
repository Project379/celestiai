...
17. **Sub-round 1 ratified `tunnelRoute: skip` based on conservative-defaults posture and "ad-blocker prevalence unknown" reasoning. Sub-round 4 forced-throw verification revealed two distinct stacked issues:**

    **(1) CSP `connect-src` blocks direct `*.ingest.sentry.io` transmission from the browser.** Clerk's strict-mode CSP at `apps/web/middleware.ts:47-53` owns the connect-src allowlist; Sentry's ingestion endpoint isn't in it. Resolved by enabling `tunnelRoute: '/monitoring'` in `withSentryConfig` — same-origin proxy bypasses CSP entirely (allowed by `'self'`) AND gives ad-blocker resistance. Matcher analysis confirmed `/monitoring` is not in `isProtectedRoute`, so no Clerk-middleware exemption needed. Verified via direct curl with the required query params (`o`, `p`, `r`) — proxy fired and Sentry returned proper auth-error response.

    **(2) DevTools "Offline" throttling is not a valid test method for client-side error-with-Sentry-receipt verification.** "Offline" blocks ALL network including `/monitoring` tunnel transmission, so Sentry SDK queues events but never transmits — and the queue doesn't reliably retry across offline-online transitions. **Correct method:** DevTools request-URL blocking (right-click request → "Block request URL") fails only the target endpoint while keeping `/monitoring` transmission available. Synthetic browser-side fetch to `/monitoring?o=...&p=...&r=de` returned 401 from Sentry's auth layer, proving end-to-end browser→tunnel→Sentry path independent of the offline-test confusion.

    **Trigger that fired:** the *"revisit post-launch with real-data signals"* clause documented in sub-round 1's close. The data signal: CSP rejection visible in browser console during ERR-DI-008 offline-test attempt (pre-launch verification phase, not post-launch — the gate caught it earlier than the trigger language anticipated, which is the point).

    **Resolution: option (b) tunnelRoute, ratified in §10.4** with matcher analysis confirming `/monitoring` is not in `isProtectedRoute` so no Clerk-middleware exemption needed. Single 1-line addition in `next.config.js` to `withSentryConfig`'s second-argument options block.
...
18. **Pre-launch prereq framing was applied to web launch when strategic plan was mobile-launch with web as parallel channel.** Multiple workstreams (§8 diary persistence, §10 error monitoring, planned §11 GDPR full-scope) optimized inside the wrong frame. Identified 2026-04-27 after founder asked about *"actual web app migration"* and provided `MOBILE_UX_RESEARCH.md` + `STACK.md` as the strategic-anchor docs. **Root cause:** advisor never requested full strategic-context enumeration at thread start; relied on partial context from claude-mem + founder ad-hoc references. The §10 + planned §11.2 UI work would have shipped web-only and required rebuild on Expo, duplicating effort against the mobile-first strategic plan.

    **Resolution:**
    - **(a) §11 scope refined** — legal docs (§11.1 privacy/ToS drafts + §11.5 ROPA) proceed in parallel, counsel-bound, don't block Phase A. UI work (§11.2) deferred until after Phase A mobile scaffolding lands so `/privacy-settings` is built universal-first per MOBILE_UX_RESEARCH §10 (*"any new component intended for both surfaces written against universal primitives"*). §11 close criterion shifts: workstream closes when all GDPR controls (legal docs + UI) ship to live paths, even if Phase A is in the critical path.
    - **(b) Context-assembly turn added as standing pattern** before any new workstream opens — advisor must request enumeration of strategic / planning corpus, not assume current context is sufficient. Index includes: `.planning/research/` strategic docs, ADRs, prior-workstream summaries, mobile-specific docs, stack/infra, memory rules.
    - **(c) Workstream-opening prompts should verify "applies to web / mobile / both" classification** — single-surface assumption is the failure mode at workstream-frame level.

    **Pattern instance — same family as #15 / #16 / #17.** Advisor pattern of operating inside a default frame without verifying the frame still holds. #15 was branch hygiene without merge-evidence; #16 was wrapper symmetry without emit-site distribution; #17 was conservative-default without environment verification; #18 is workstream framing without strategic re-anchoring. **The common rule: whenever a default would silently shape multi-round work, verify the default first.** #18 closes the pattern at the strategic-frame layer — the most expensive place for it to fire (multi-workstream cost vs single-decision cost in #15-#17).

19. **`apps/mobile/metro.config.js` from Apr 18 scaffold (`db8a3e4`) used `disableHierarchicalLookup = true` — the standard Expo monorepo pattern for yarn/npm flat `node_modules` layouts but incompatible with pnpm's `.pnpm/` virtual store sibling-symlink resolution.** Surfaced 2026-04-28 during Phase A sub-round 1 commit 1.3 verification when `pnpm --filter @celestia/mobile start --clear` failed with `Unable to resolve module @react-navigation/native from expo-router/build/static/renderStaticContent.js`. The package was present at `node_modules/.pnpm/@react-navigation+native@7.2.2_<peer-hash>/node_modules/@react-navigation/native` and properly linked into `expo-router`'s sibling `node_modules/`; Metro just couldn't walk there because hierarchical lookup was disabled. PITFALLS.md #4 explicitly documents the override risk ("don't manually override `watchFolders`, `nodeModulesPaths` unless necessary") but the Apr 18 scaffold predated active `--clear` verification of pnpm-compatibility for the override pattern. **Causation chain:** latent since Apr 18; surfaced when commit 1.1's heavy install reshuffled `.pnpm/` content-addressed paths (peer-dep hashes regenerated when `@clerk/expo`, `@supabase/supabase-js`, `expo-secure-store`, `expo-crypto` joined the dep graph), invalidating any previously-cached Metro resolution; commit 1.3's verification step required `--clear` which forced the cache flush that exposed the resolver gap. Either factor alone might have hidden the issue indefinitely; the conjunction made it visible. **Resolution:** single-line removal of `disableHierarchicalLookup` (Option A1, surgical); preserves `watchFolders` + `nodeModulesPaths` overrides as explicit-intent breadcrumbs per the conservative-defaults posture.

    **Pattern instance — same family as #15 / #16 / #17 / #18, with a new temporal dimension.** Advisor pattern of operating inside a default frame without verifying the frame still holds. **#15-#18 all involved a default set inside an active workstream** where the default could be challenged at frame-set time. **#19 is structurally newer:** the default was set in a scaffold commit (`db8a3e4`, Apr 18, ~10 days before Phase A sub-round 1 work began), and the verifying surface (`expo start --clear`) wasn't exercised in the intervening rounds. **Pattern observation: scaffolds carry assumptions; assumptions go stale when the surrounding environment shifts (pnpm version, dep tree shape, lockfile hash topology).** Re-verification discipline must extend to scaffold artifacts whenever those artifacts touch surfaces that depend on environmental conditions (bundler resolution, build pipelines, lockfile hash topology). Adds **at-environment-shift-time** to the prior **at-decision-time** locus. Practical extension of the rule from #18: whenever a workstream re-opens against scaffold from a prior round, the scaffold's environment-dependent assumptions get a quick re-verify pass before substantive work fires.

20. **Posture A peer-deferral assumed peers were optional at compile time, but SDKs that use static-analyzable dynamic imports require peers present in `node_modules` at bundle time even when features aren't runtime-used.** Surfaced 2026-04-28 during Phase A sub-round 1 commit 1.3 verification round-trip 4, when `pnpm --filter @celestia/mobile start --clear` failed with `TypeError: The "to" argument must be of type string. Received undefined` at `@expo/metro-config@0.20.18/build/serializer/fork/js.js:72` — `path.relative(serverRoot, dependency.absolutePath)` where `dependency.absolutePath = undefined`. Three prior speculative fixes in the same debug round missed (parent layout `_layout.tsx`, metro `disableHierarchicalLookup` removal, web target removal from `app.json`); the breakthrough came from reading the actual `@expo/metro-config` source at the cited line and walking the dependency chain backward. Root cause: `@clerk/expo@3.2.4` references peer dependencies via static-analyzable patterns that Metro resolves at bundle time regardless of runtime gates:

    - `provider/ClerkProvider.js:280` — `require("expo-web-browser")` (synchronous, in try/catch, gated by `if (isWeb())`)
    - `hooks/useOAuth.js:55` — `await Promise.all([import("expo-auth-session"), import("expo-web-browser")])`
    - `hooks/useSSO.js:53` — same as `useOAuth`
    - `hooks/useSignInWithApple.ios.js:51` — `await Promise.all([import("expo-apple-authentication"), import("expo-crypto")])`
    - `passkeys/index.js:24` — `require("@clerk/expo-passkeys")` (synchronous)

    Per Posture A registered at commit 1.1, all four peers (`expo-web-browser`, `expo-auth-session`, `expo-apple-authentication`, `@clerk/expo-passkeys`) were deferred — the discipline reasoned "if no runtime feature uses it, don't install it." The dynamic require/import calls in `@clerk/expo` source are gated by runtime checks (`isWeb()`, feature-flag patterns) so they would never EXECUTE without the feature being in use. But Metro's static analyzer doesn't see those gates — it sees the import string literal, tries to resolve it during bundle generation, fails, and creates a dependency entry with `absolutePath = undefined`. The serializer's optional-handling branch at `js.js:41` reassigns a local `modulePath` variable but leaves `dependency.absolutePath` itself unchanged, so the downstream call at line 72 still receives undefined and crashes. Web-only failure (native compiles cleanly at 892 modules) is consistent with line 72 being gated on `options.includeAsyncPaths && options.sourceUrl` which are web-typical settings; native bundling skips this code path.

    **Resolution:** install all 4 peers despite no current runtime feature use. Bundle size impact ~140KB (negligible for mobile). REVISIT-TRIGGERS.md items 2/3/4 reframed from "deferred until feature lands" to "peer installed at bundle time; runtime feature deferred until X." Wire-up paths preserved for future feature work.

    **Posture A refinement (institutional lesson):** the original posture — *"opt-out of third-party SDK richness pre-launch; expand on evidence with named revisit triggers"* — produced this evidence and remains the right discipline. **The refinement: Posture A's install criterion needs a second axis.** Original criterion was "do we use this peer at runtime?" The refinement adds: "OR does the SDK statically reference this peer in source code Metro will analyze?" Both axes must be checked before deferring a peer. Practical pattern for future install decisions: before applying Posture A to a peer, grep the SDK's compiled source for `require("<peer>")`, `import("<peer>")`, and `from "<peer>"` patterns. If any are present, the peer must be installed at compile time regardless of runtime feature use. Sibling of #19 (scaffold defaults misalign with stack reality) and #15-#18 (default frame held without verification) — this one is at the **install-decision frame**, where conservative-defaults were applied without verification of the SDK's static-analysis surface.

    **Three-speculative-fix lesson (procedural).** During the same debug round, three fixes landed before this one based on surface-symptom reading (parent layout, metro hierarchical-lookup, web target removal). The first was structurally needed independently of the bundle error; the second resolved a separate latent issue; the third was wasted work. The breakthrough required reading the actual `@expo/metro-config` source at the error's cited line — which had been ambiguous because the source-mapped line number (`Sitemap.js:144`) pointed to innocuous styles and misled the diagnosis. **Procedural rule: when a stack trace's cited line doesn't match a plausible call site on visual inspection, the trace is source-mapped to a different file or the line number is stale; read the compiled `.js` (not the `.ts` source) to find the actual call.** Sibling discipline to drift #13's "advisor reasoning treated as verified when it was inferred" — the cure is the same: read the file, don't infer from the trace summary.

Full cleanup pass deferred to post-§9.6 — this breadcrumb exists so future readers searching for "swisseph-wasm," "topocentric Moon," "True Node precision," "historical-tz / LMT," "Mean Node reference source," "astro.com independence," "DST-transition ambiguity," "DE441 upper bound," "packages/core AGPL drift / pnpm overrides," "clerk_id helper / diary RLS," "auth.jwt() in DEFAULT / subquery feature not supported," "migration-history drift / out-of-band DDL channel," "RLS as primary gate vs service-role actual," "Next.js serverExternalPackages / sweph native-module bundler," "advisor ceremony recommendation / mobile-parallel-test as active trunk," "advisor double-tagging recommendation / client-vs-server emit-site distribution," "tunnelRoute / CSP connect-src blocking client-side Sentry transport," "DevTools offline throttling as invalid test method for client-side network-failure Sentry receipt verification," "advisor frame-default pattern / web-vs-mobile workstream-framing without strategic re-anchoring," or "metro.config.js disableHierarchicalLookup / pnpm virtual store sibling resolution / scaffold-set assumption staleness," or "Posture A peer-deferral / SDK static-analyzable dynamic imports / @clerk/expo + @expo/metro-config serializer path.relative undefined / Metro static analysis ignores runtime gates around require / install-decision second axis" find the trail.
