...
14. **Next.js dev server crashed on `/api/planets/current` (and every other sweph-importing endpoint) with `TypeError: The "path" argument must be of type string or an instance of URL. Received an instance of URL` — self-contradicting message typical of Node's native-module resolver receiving a `file://` URL where a string path was expected inside a Webpack-bundled require chain.** Discovered 2026-04-21 when founder attempted to run the §8.4 UAT harness against a locally-started dev server. `[verified]` via `node -e "require('sweph')..."` from `packages/astrology/` that `sweph@2.10.0-11` loads cleanly at Node v24.12.0 runtime and `calc_ut` returns real data — **the native module itself is fine.** `[verified]` CI (Vitest) on current HEAD is green across 3 most-recent `Astrology Validation` runs — Vitest bypasses Webpack, so the CI path never hits this. `[verified]` `apps/web/next.config.js` had no `serverExternalPackages` entry (Next 15 stable) nor `experimental.serverComponentsExternalPackages` (Next 13/14) listing `sweph` before this fix. Resolved by adding `serverExternalPackages: ['sweph']` at the top level of `next.config.js` — tells Next.js to externalize `sweph` so it's `require()`'d natively at runtime. `[inferred]` trigger is one (or both) of: (a) the §9A workspace-wide pnpm override (`6bb4ff2`) regenerating `pnpm-lock.yaml` and re-resolving native bindings differently; or (b) a local Node version update to 24.12 (root package.json's `engines.node: ">=22.0.0"` allows it, but Node 24's resolver type-checks file-URL arguments more strictly). **Not ruled definitively** — `serverExternalPackages` is trigger-agnostic; either trigger would be masked by the fix. Post-launch discipline reminders queued: if (a) — any `pnpm.overrides` change should include a dev-server smoke-test in the close checklist; if (b) — `engines.node` should be narrowed from `>=22.0.0` to a specific major or range that's been verified across dev + CI + prod runtime, and `.nvmrc` / equivalent should lock the dev version. Classification: environmental regression orthogonal to any §8 work; the diary module graph doesn't import `sweph` and the §8.4 endpoints compile clean.

Full cleanup pass deferred to post-§9.6 — this breadcrumb exists so future readers searching for "swisseph-wasm," "topocentric Moon," "True Node precision," "historical-tz / LMT," "Mean Node reference source," "astro.com independence," "DST-transition ambiguity," "DE441 upper bound," "packages/core AGPL drift / pnpm overrides," "clerk_id helper / diary RLS," "auth.jwt() in DEFAULT / subquery feature not supported," "migration-history drift / out-of-band DDL channel," "RLS as primary gate vs service-role actual," or "Next.js serverExternalPackages / sweph native-module bundler" find the trail.
