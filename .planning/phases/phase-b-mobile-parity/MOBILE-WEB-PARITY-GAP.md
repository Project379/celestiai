...
| 7.4 | Push subscription — web-push browser opt-in (Phase 6.3 work) + service worker + cron at 06:00 UTC | `apps/web/components/horoscope/PushNotificationBanner.tsx` + `apps/web/public/sw.js` | **[VERIFIED, 2026-08-16, real-status sweep]** stale — this row said REVISIT-26's schema+endpoint were still required; both are built and live: `supabase/migrations/20260803070000_push_tokens.sql` (RLS, FK, unique-per-device), `apps/web/app/api/push/register/route.ts` (rate-limited upsert, Batch 5.5 #20), `apps/web/app/api/cron/daily-horoscope/route.ts` (sends via `expo-server-sdk`, handles `DeviceNotRegistered`, revokes dead tokens), scheduled in `apps/web/vercel.json`. Missed when `COMPLETION-TRACKER.md`'s halt-required register and REQUIREMENTS.md's DAILY-04 were corrected the same day — this row was the same gap, just not caught in the same sweep. | **done (backend) — only the permission/settings UI remains, Batch 8.** Not a halt-required item; REVISIT-26 is resolved. |
