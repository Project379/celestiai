...
| 3.4 | (NEW — 2026-08-13) Full Кръг feature port: relationship types, compatibility domains, saved-people profiles, connection invites, relationship reports/weather | `apps/web/components/circle/{CircleHub,SavedProfileForm,ConnectInviteAcceptance}.tsx` + `apps/web/app/api/circle/**` + `apps/web/lib/circle/*` (~2,200 LOC reference surface) | absent entirely — `circle.tsx` is still the pre-port empty-state placeholder | **not started.** Tracked in detail at `STREAM-K-PORT-LOG.md`; that doc explicitly flags "No mobile surface exists at all... a separate, much larger port whenever that's prioritized." |
...
## Section 11: API rate-limiting coverage (NEW — 2026-08-13)

**Not a mobile-vs-web parity gap** — both surfaces share the same `apps/web/app/api/**` routes, so this affects them equally. Flagged here per this refresh's investigation scope because it's adjacent security-debt worth tracking somewhere, and this is the living inventory that gets read before batch-sequencing work. [verified] via `apps/web/lib/rate-limit.ts` (`assertRateLimit`, table-backed, fail-open on limiter error) and a grep for its call sites across all 34 `apps/web/app/api/**/route.ts` files.

**Protected (7 of 34 route files call `assertRateLimit`):** `diary/entries`, `circle/relationships/[relationshipId]/weather`, `circle/relationships/[relationshipId]/report`, `circle/profiles/[profileId]/report`, `oracle/generate`, `horoscope/generate`, `cities/search`.

**Unprotected — includes several sensitive/expensive routes, called out specifically per this session's earlier finding that `birth-data` ships with no rate limit at all:**
- `birth-data/route.ts` and `birth-data/[id]/route.ts` — **[verified] still unprotected.** Write path for user birth data; no abuse ceiling.
- `chart/calculate/route.ts` — **[verified] still unprotected.** This is the Swiss-Ephemeris WASM compute route (the one CLAUDE.md flags as the heavy server-side calculation) — unmetered.
- `gdpr/delete-account/route.ts` and `gdpr/export/route.ts` — unprotected. Destructive/export account action with no throttle.
- `stripe/checkout`, `stripe/portal`, `stripe/cancel`, `stripe/subscription`, `stripe/status` — all 5 unprotected.
- `oracle/readings/route.ts` — unprotected (note: `oracle/generate` IS protected; the readings-list endpoint is not).
- `push/register`, `push/subscribe`, `push/unsubscribe` — unprotected.
- `circle/invites/*`, `circle/profiles/route.ts`, `circle/profiles/[profileId]/route.ts`, `circle/relationships/[relationshipId]/archive` — unprotected (only the 3 report/weather circle routes have it; invite creation/acceptance and profile CRUD do not).
- `user/route.ts`, `crystals/*` (4 routes), `planets/current`, `transits/overview`, `webhooks/stripe`, `webhooks/revenuecat`, `cron/*` (2 routes) — unprotected (webhooks/cron are arguably fine — different trust model — but not asserted anywhere, so this is a judgment call rather than a checked gate).

This doc does not carry an opinion on which of these need it before soft launch — that's a founder call — but `birth-data` and `chart/calculate` are the two most exposed given they're unauthenticated-adjacent-cost surfaces on the compute path CLAUDE.md itself calls out as the expensive one.
